 ...
 *
 *
 */ //...rest ...spread, .toFixed(2) - rotungeste la 2 cifre dupa virgula

/// --------------- My Option ---------------

function meanScore(...arguments) {
  let together = [...arguments];
  const avAmount =
    together.reduce((acc, val) => acc + val, 0) / together.length;
  if (isNaN(avAmount)) {
    return `Все аргументы в вызове функции должны быть числами!`;
  }
  return Math.round(avAmount * 100) / 100;
}
